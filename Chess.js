...
function vervangMetPlaatje() {
    document.querySelectorAll('.geslagen').forEach(image => {
        ...
        if (image.innerText.length !== 0) {
            if (image.innerText == 'Wpawn' || image.innerText == 'Bpawn') {
                image.innerHTML = `${image.innerText} <img class='allimg allpawn' src="${image.innerText}.png" alt="" width="15">`;
...
// geslagen stuk opslaan in tkp
                    ...
                    document.getElementById('tkp').innerHTML += '<span class="geslagen" style="font-size:0px" >' + item.innerText + '</span>';
               //     vervangPlaatje
                    vervangMetPlaatje();
